...
class HuggingFaceMultiModal(MultiModalLLM):
    ...
    def complete(
        ...
        prepared_inputs = self._prepare_messages(
            [ChatMessage(role="user", content=prompt)], image_documents, logits_processors=logits_processors
        )
        generated_text = self._generate(prepared_inputs, logits_processors=logits_processors)
...
class Qwen2VisionMultiModal(HuggingFaceMultiModal):
    ...
    def _prepare_messages(
        self, messages: Sequence[ChatMessage], image_documents: Sequence[ImageDocument], logits_processors: LogitsProcessorList = None
    ) -> Dict[str, Any]:
        """
    ...
    def _generate(self, prepared_inputs: Dict[str, Any], logits_processors: LogitsProcessorList = None) -> str:
        """
        Generates text based on prepared inputs. The text is decoded from token IDs generated by the model.
        """
        output_ids = self._model.generate(
            **prepared_inputs, max_new_tokens=self.max_new_tokens, logits_processor=logits_processors
        )
        generated_ids = [
...
class Phi35VisionMultiModal(HuggingFaceMultiModal):
    ...
    def _prepare_messages(
        self, message: ChatMessage, image_documents: Sequence[ImageDocument], logits_processors: LogitsProcessorList = None
    ) -> Dict[str, Any]:
        """
...
class LlamaMultiModal(HuggingFaceMultiModal):
    ...
    def _prepare_messages(
        ...
        # Some processor requires internal state resets based on LLM prompts
        if logits_processors:
            for processor in logits_processors:
                if hasattr(processor, "reset"):
                    processor.reset(input_text)
